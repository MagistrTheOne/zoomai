 ...
 * @param {{ pulseSlot?: number }} [extra]
 */
function buildLaunchOptions(headless, extra = {}) {
  const commonArgs = [
  ...
  if (process.env.AUDIO_OUT_MODE === "virtual_mic") {
    const slot = extra.pulseSlot ?? Number(process.env.PULSE_AUDIO_SLOT ?? 0);
    opts.env = {
      ...process.env,
      PULSE_SOURCE: `nullxes_mic_${slot}`,
    };
  }
